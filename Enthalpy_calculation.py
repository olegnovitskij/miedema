...
def unfold(l):
    '''
    выпрямление списка спиков[[1, 2, 3], [4, 5, 6], ...] в один список [1, 2, 3, 4, 5, 6,...]
    '''
...
def H_miedema (A: str, B: str, 
               ...
               miedema_coifficients_path='miedema_coefficients.xlsx', 
               **kwargs)->float:
    '''
    A, B - названия элементов из miedema_coifficients_path
    type_of_compound - тип соединения 1-неупорядоченный тв р-р,2-упорядоченное соединение,3-аморфные фазы
    num - число разбиений сетки концентраций от 0 до 1
    mode - брать среднее 'mean' или 'minmax' экстремальное значение
    radius_type - 'atom' атомный радиус и 'ion' ионный радиус
    miedema_coifficients_path - путь до коэффициентов
    '''
...
def Morse_potential(x, n=12, m=6):
    '''
    n, m - коэффициенты Л-Дж соответствующие потенциалу Морзе
...
def make_calculation_for_one_row(dist_arr_row, all_params_combinations=None, type_compound=2, is_abs_potential=False):
    '''
    строка для расчёта
    all_params_combinations - все комбинации параметров
    type_compound - тип соединения
    is_abs_potential - брать модуль ямы или значение со знаком
    '''
...
def calculate_enthalpy(dist_arr: np.array, 
    ...
    '''
    составление всех комбинаций парамтеров и расчёта энергий для всех строк в dist_arr
    '''
...
def convert_to_float(frac_str):
    '''
    функция для замены , на . при считывании
    а также перевода чисел с косой чертой в числа с плавающей запятой. 1/3 в 0.333333...
    '''
    frac_str.replace(',', '.')
    try:
