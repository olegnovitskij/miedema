...
class VisualizationWindow(Screen):
    ...
    def remove_element_button(self):
        if self.ids.n != 0:
            for label in ["el", "x", "y", "z", "wp", "sym", "occ"]:
                exec(
                    f"self.ids.el_table.remove_widget(self.ids.el_table.ids.{label}_{self.ids.n})"
                )
    ...
    def press_restart_button(self):
        self.h.text = f"Doing..."
        self.ids.extended_grouped_dist_arr = copy.deepcopy(
            self.manager.ids.main.ids.grouped_dist_arr
        )
        ...
        for el, wp, occ in el_list:
            cond1 = (
                (
                    (self.ids.extended_grouped_dist_arr[:, 1] == wp)
                    & (self.ids.extended_grouped_dist_arr[:, 0] == el)
                )
                .any()
                .any()
            )
            ...
            cond2 = (
                (
                    (self.ids.extended_grouped_dist_arr[:, 3] == wp)
                    & (self.ids.extended_grouped_dist_arr[:, 2] == el)
                )
                .any()
                .any()
            )
...
class MainWindow(Screen):
    ...
    def press_button(self):
        ...
        self.crystal_info = (
            pd.DataFrame(crystal_list, columns=columns_crystal)
            .replace(r"^\s*$", np.nan, regex=True)
            .dropna()
            .astype("float64")
            .astype({"compound_type": "int64", "spacegroup": "int64"})
        )

        self.element_info = (
            pd.DataFrame(el_list, columns=columns_el)
            .replace(r"^\s*$", np.nan, regex=True)
            .dropna()
            .astype(
                {
        ...
        self.params = {
            "spacegroup": int(self.crystal_info.loc[:, "spacegroup"].values[0]),
            "basis": [
                tuple(x) for x in self.element_info.loc[:, ["x", "y", "z"]].values
            ],
            "symbols": self.element_info.loc[:, "el"].values,
            "cellpar": self.crystal_info.loc[
        ...
        try:
            ...
            grouped_dist_df = (
                ...
                .groupby(["A", "A_site", "B", "B_site", "Distance", "occ_A", "occ_B"])[
                    "Distance"
                ]
                .count()
                .to_frame("cnt")
            ...
            grouped_dist_df["dupl_set"] = grouped_dist_df.apply(
                lambda x: frozenset(
                    [x["A"], x["A_site"], x["B"], x["B_site"], x["Distance"]]
                ),
                axis=1,
            )
            ...
            write(str(image_path), crystal_ase)
            ...
            for num, s in enumerate(crystal_pyxtal.atom_sites):
                ...
                self.manager.ids.visu.ids.el_table.ids[f"wp_{num + 1}"] = weakref.ref(
                    wp
                )
                self.manager.ids.visu.ids.el_table.ids[f"sym_{num + 1}"] = weakref.ref(
                    sym
                )
                self.manager.ids.visu.ids.el_table.ids[f"occ_{num + 1}"] = weakref.ref(
                    occ
                )
